...
export const MainPage: React.FC = () => {
  ...
  const ownerAvatarTransform = (userId: string) => FRIENDS.find(user => user.id === userId)?.avatarTransform;
    ...
    <div>
      ...
      {rankingPagination.items.map(({ stock, returnRate }, i) => (
        <div className={`ranking-row ranking-row--${isKoreanStock(stock.symbol) ? 'domestic' : 'global'}`} key={stock.userId + stock.symbol}>
          ...
          <FriendAvatar name={ownerName(stock.userId)} src={ownerAvatar(stock.userId)} objectPosition={ownerAvatarPosition(stock.userId)} transform={ownerAvatarTransform(stock.userId)} className="ranking-avatar" />
        ...
        <div>
          {myStocks.map(stock => (
            <StockCard 
              ...
              ownerAvatarTransform={ownerAvatarTransform(stock.userId)}
    ...
    return (
      <div className="friend-list">
        ...
        {activeFriendId && friendStocks.map(stock => (
          <StockCard key={stock.userId + stock.symbol} stock={stock} ownerName={ownerName(stock.userId)} ownerAvatar={ownerAvatar(stock.userId)} ownerAvatarPosition={ownerAvatarPosition(stock.userId)} ownerAvatarTransform={ownerAvatarTransform(stock.userId)} quote={quotes[stock.symbol]} />
        ))}
